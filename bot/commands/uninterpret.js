...
module.exports = {
	meta: {
        "name": "uninterpret",
        "description": "Stop interpret messages in this channel to specific languages",
        "options": [
            {
                "type": 3,
	...
	async execute(client, e) {
        ...
        chatData.removeLanguage(language.value);
        
        await Promise.all([
            chatData.save(),
            (async function(){
                let text = await chatBot.getLocalizedMessage(`Okay! stop interpret to "${language.label}"`, {language: "en"});
                await e.reply(text);
            })(),
        ]);
        return;
